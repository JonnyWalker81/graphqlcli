...
let rec read_while condition lexer acc =
  if condition lexer.ch
  then read_while condition (read_char lexer) (acc ^ String.make 1 lexer.ch)
  else lexer, acc
;;
...
let peek lexer ch current matched =
  ...
  then lexer, Token.Eof
  else (
    let peek_char = String.get lexer.input lexer.read_position in
    if Char.to_int peek_char = Char.to_int ch
    then (
...
let peek_is lexer amount to_match =
  if lexer.position + amount >= String.length lexer.input
  then lexer, false
  else (
...
let read_ellipsis lexer =
  ...
  if is_period_one && is_period_two
  then (
    let lexer = read_char lexer in
    let lexer = read_char lexer in
    let lexer = read_char lexer in
